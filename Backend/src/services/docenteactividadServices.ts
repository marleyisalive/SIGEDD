...
export const agregarDocenteActividad = async (nueva: docenteactividad) => {
  ...
  datosParaInsertar.datosCapturados = JSON.stringify(
    nueva.datosCapturados
  ) as any;

  // 3. Insertar
  const [results] = await conexion.query("INSERT INTO docenteactividad SET ?", [
    datosParaInsertar,
  ]);
  return results;
};
...
export const actualizarDocenteActividad = async (
  ...
  try {
    ...
    if (datosAActualizar.datosCapturados) {
      (datosAActualizar as any).datosCapturados = JSON.stringify(
        datosAActualizar.datosCapturados
      );
    }
