...
router.delete("/", async (req: Request, res: Response) => {
  try {
    ...
    const eliminada = await docenteActividadServices.eliminarDocenteActividad(
      idDocenteActividad
    );

    // Verificar si hay error
    if ("error" in eliminada) {
      return res.status(400).send({ error: eliminada.error });
    }
    ...
    if (err.errno === 1451 || err.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(400).send({
        error:
          "No se puede eliminar esta actividad de docente porque tiene registros asociados (como documentos). Elimine primero los registros relacionados.",
      });
    }
