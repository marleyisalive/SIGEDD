...
// Obtener actividades por ID de Usuario (Docente)
// Esta es la ruta que llama tu pantalla de validación
router.get("/usuario/:idUsuario", async (req: Request, res: Response) => {
  try {
    const idUsuario = Number(req.params.idUsuario);
    // Llamamos al servicio que YA TIENES creado (obtenerActividadesPorUsuario)
    const actividades = await docenteActividadServices.obtenerActividadesPorUsuario(idUsuario);
...
// --- APROBAR DOCUMENTO (PUT) ---
router.put("/aprobar/:id", async (req: Request, res: Response) => {
  try {
    const idActividad = Number(req.params.id);
    const { idAdministrativo } = req.body;

    const resultado = await docenteActividadServices.aprobarActividad(idActividad, idAdministrativo);
    ...
    return res.send({ mensaje: "Aprobado correctamente", resultado });

  } catch (err) {
    console.error("Error en ruta aprobar:", err);

    return res.status(500).send({ error: "Error interno." });
  }
});

// --- RECHAZAR DOCUMENTO (PUT) ---
