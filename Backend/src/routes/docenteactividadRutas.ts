...
router.get(
  "/:idDocenteActividad/comision-estrategicos",
...
router.get(
  "/:idDocenteActividad/constancia-rh",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
...
router.get(
  "/:idDocenteActividad/exclusividad",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosCartaExclusividad(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/alumnos-atendidos",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaAlumnos(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/codireccion",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
...
router.get(
  "/:idDocenteActividad/horario",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosHorarioActividades(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get("/:idDocenteActividad/pit", async (_req: Request, res: Response) => {
  try {
    ...
    if (!datos) {
      res
        .status(404)
        .send({ error: "No se encontraron datos para generar la constancia." });
...
router.get(
  "/:idDocenteActividad/acreditacion",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaAcreditacion(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/complementaria",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaComplementaria(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/manual",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosConstanciaManual(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
...
router.get(
  "/:idDocenteActividad/estrategia",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaEstrategia(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/diplomado",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaDiplomado(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/comision",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosOficioComision(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
...
router.get(
  "/:idDocenteActividad/comision-tecnm",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosOficioComisionTecNM(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/comision-pensamiento",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosOficioPensamientoCritico(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/comision-ambientes",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosOficioAmbientesVirtuales(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/productos",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaProductos(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/exencion",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaExencion(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/propuesta",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosPropuestaPrograma(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/sabatico",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosOficioSabatico(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
...
router.get(
  "/:idDocenteActividad/liberacion",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosLiberacionActividades(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/liberacion-academica",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosLiberacionAcademica(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/reporte-investigacion",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosReporteInvestigacion(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/evaluacion-docente",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosEvaluacionDocente(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/horario-adicional",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
...
router.get(
  "/:idDocenteActividad/horario-posgrado",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
...
router.get(
  "/:idDocenteActividad/estudiantes-114",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
...
router.get(
  "/:idDocenteActividad/constancia-proyectos",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaProyectos(id);
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
        return;
      }
...
router.get(
  "/:idDocenteActividad/asesoria-cb",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosAsesoriaCienciasBasicas(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/asesoria-generic",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosAsesoriaGeneric(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
...
router.get(
  "/:idDocenteActividad/concurso-generic",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaConcurso(id);
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
        return;
      }
...
router.get(
  "/:idDocenteActividad/jurado-generic",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosConstanciaJurado(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
...
router.get(
  "/:idDocenteActividad/comite-generic",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosConstanciaComite(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
...
router.get(
  "/:idDocenteActividad/auditoria-generic",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaAuditoria(id);
      if (!datos) {
        res.status(404).send({ error: "No encontrado" });
        return;
      }
...
router.get(
  "/:idDocenteActividad/comision-inclusiva",
  async (_req: Request, res: Response) => {
    const id = Number(_req.params.idDocenteActividad);
    const datos = await docenteActividadServices.obtenerDatosOficioGenerico(id);
...
router.get(
  "/:idDocenteActividad/investigacion-05",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos =
        await docenteActividadServices.obtenerDatosConstanciaInvestigacion(id);
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
        return;
      }
...
router.get(
  "/:idDocenteActividad/licencia-gravidez",
  async (_req: Request, res: Response) => {
    try {
      const id = Number(_req.params.idDocenteActividad);
      const datos = await docenteActividadServices.obtenerDatosLicenciaGravidez(
        id
      );
      if (!datos) {
        res.status(404).send({ error: "No se encontraron datos." });
...
// --- OBTENER ACTIVIDADES POR USUARIO (GET) ---
// GET http://localhost:3001/api/docenteactividad/usuario/:idUsuario
router.get("/usuario/:idUsuario", async (req: Request, res: Response) => {
  try {
    const idUsuario = Number(req.params.idUsuario);
    let actividades =
      await docenteActividadServices.obtenerActividadesPorUsuario(idUsuario);
    res.send(actividades);
  } catch (err) {
