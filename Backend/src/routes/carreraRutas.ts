...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    let carrera = await carreraServices.encontrarCarreraPorId(
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const nueva = await carreraServices.agregarCarrera({
      idCarrera,
      nombre,
      acreditado,
    });
    ...
    if (err.code === "ER_DUP_ENTRY" || err.errno === 1062) {
      res.status(400).send({
        error: "El ID de la carrera ya existe. Por favor, use un ID diferente.",
      });
    } else {
...
router.put("/", async (req: Request, res: Response) => {
  try {
    const { idCarrera, nombre, acreditado } = req.body;
    const modificada = await carreraServices.actualizarCarrera({
      idCarrera,
      nombre,
      acreditado,
    });
...
router.delete("/", async (req: Request, res: Response) => {
  try {
    ...
    if (err.code === "ER_ROW_IS_REFERENCED_2" || err.errno === 1451) {
      res.status(400).send({
        error:
          "No se puede eliminar la carrera porque tiene registros asociados. Elimine primero los registros relacionados.",
      });
    } else {
