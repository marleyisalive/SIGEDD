<template>
  <div class="container mt-5" v-if="tipoDocumento[0]">
    <div class="card">
      ...
      <div class="card-body">
        <Form
          :validation-schema="TipoDocumentoSchema"
          :initial-values="tipoDocumento[0]"
          @submit="onTodoBien"
        >
          <div class="mb-3">
...
onMounted(async () => {
  ...
  // Cargar los campos existentes desde el JSON
  if (tipoDocumento.value[0]?.plantillaJSON) {
    try {
      const parsed = JSON.parse(tipoDocumento.value[0].plantillaJSON);
      if (parsed.campos && Array.isArray(parsed.campos)) {
        campos.value = parsed.campos;
      }
    } catch (error) {
      console.error("Error al parsear plantillaJSON:", error);
    }
  }
});

const onTodoBien = async (values: any) => {
  await actualizarTipoDocumento(values);
};
</script>
